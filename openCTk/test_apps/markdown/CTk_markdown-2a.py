...
def code_create(event):
    ...
    format_window(chat_response)  # Call the format_window function with chat_response as an argument

# Updated function with an additional parameter
def format_window(chat_response):
    ...
    text_widget.insert(CTk.INSERT, html_content)
    text_widget.pack()
...
btn_gen_mark = customtkinter.CTkButton(master=code_gen_frame, width=20, text="Format", command=lambda: format_window(""))
