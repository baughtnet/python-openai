...
text_frame.grid(row=0, column=1, rowspan=2, padx=(5, 30), pady=5, sticky='nsew')
...
txt_gpt.grid(row=0, column=0, padx=5, pady=(5, 2), sticky='nsew')

# Create the smaller frame for lbl_prompt and txt_prompt
prompt_frame = customtkinter.CTkFrame(master=text_frame)
prompt_frame.grid(row=1, column=0, padx=5, pady=2, sticky='nsew')
prompt_frame.columnconfigure(0, weight=8)
prompt_frame.columnconfigure(1, weight=1)

# Create the "Prompt:" label
lbl_prompt = customtkinter.CTkLabel(master=prompt_frame, text="Prompt:", height=1)
lbl_prompt.grid(row=0, column=0, padx=(0, 2), pady=5, sticky='nsew')

# Create the smaller textbox for txt_prompt
txt_prompt = customtkinter.CTkTextbox(master=prompt_frame, wrap='word')
txt_prompt.grid(row=0, column=1, padx=(2, 5), pady=5, sticky='nsew')

# Create the send button
btn_chat = customtkinter.CTkButton(text_frame, text="Send", command=lambda: update_win(""))
btn_chat.grid(row=1, column=0, padx=(5, 0), pady=(2, 5), sticky='nsew')
...
# Calculate the width and height for the textboxes
...
txt_gpt_height = int(total_height * 0.65)
txt_prompt_height = int(total_height * 0.2)

# Set the width and height of the textboxes
txt_gpt.configure(width=txt_gpt_width, height=txt_gpt_height)
txt_prompt.configure(width=txt_prompt_width, height=txt_prompt_height)
