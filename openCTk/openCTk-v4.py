...
def update_html(prompt_txt, chat_response):
    ...
    txt_gpt.insert("end", html_content)

    if html_window and html_window.winfo_exists():
...
def float_txt(event):
    ...
    float_window.geometry("300x100")

    txt_float = Entry(float_window)
    txt_float.pack(padx=10, pady=10, fill="both", expand=True)
    txt_float.focus()

    content = txt_float.get(1.0, END)

    btn_send = customtkinter.CTkButton(float_window, text="Send", command=lambda: chat("", content))

def chat(event, prompt_txt, content):
    # global content
    # content = txt_prompt.get(1.0, "end-1c")
    # txt_gpt.insert("end", "You say:  " + content + "\n" + "---------------------------------------------------------------" + "\n")
    prompt_txt = prompt_txt + "<h3>Welcome to ChatGPT Custom</h3><br><hr><br> <h4>You say:</h4>  "
...
prompt_txt = txt_prompt.get(1.0, "end-1c")

btn_chat = customtkinter.CTkButton(master=main_frame, width=20, text="Send", command=lambda: chat("", prompt_txt, ""))
